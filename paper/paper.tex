...
We find that the models reproduce conditional economic structure remarkably well---education--income gradients within 5.1--11.6\% mean absolute relative error, Mincer wage regressions with $R^2 = 0.87$--0.90, and near-perfect internal consistency between employment, hours, and income---but systematically distort unconditional marginals, overstating the employment rate (62--83\% vs.\ 63\% reference) and the share of advanced degrees.
A three-condition prompt ablation isolates the contribution of the models' \emph{latent} economic knowledge from \emph{explicit} priors supplied in the prompt, revealing that explicit priors substantially improve income fidelity (MARE reduction of 36--77\%) while having limited effect on econometric utility ($R^2$ changes $< 0.04$).
Compared with two non-LLM baselines (uniform and marginal sampling), the LLMs' unique value lies in their joint structure ($R^2 \geq 0.86$), not their marginals---a marginal sampler achieves MARE of 4.0\% but $R^2$ of only 0.23.
We then demonstrate that the paper's recommended remedy works: a single post-stratification step that reweights records to match the public education distribution drives the education-distribution total variation distance to zero while leaving the Mincer $R^2$ essentially unchanged (within $\pm 0.03$), confirming that the LLMs' joint structure survives marginal calibration.
...
Solar Pro 4 achieves the lowest MARE (5.1\%) and TVD (0.104), closely followed by Qwen 3.7 Flash (MARE 9.6\%, TVD 0.138) and Ling 3.0 Flash (MARE 9.4\%, TVD 0.156).
...
All four LLMs substantially outperform the uniform baseline (MARE 97.9\%) but are outperformed by the marginal baseline on MARE (4.0\%)---a comparison that is central to our interpretation (Section~\ref{sec:baselines}).
...
Figure~\ref{fig:education} visualizes the systematic over-representation of advanced degrees relative to the Census reference---the marginal distortion that motivates the calibration step in Section~\ref{sec:calibration}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/education_distribution.png}
\caption{Educational attainment: synthetic (priors) vs.\ Census reference and baselines. All four models over-represent advanced degrees (bachelor's and above) relative to the reference.}
\label{fig:education}
\end{figure}
...
Solar Pro 4        & \textbf{5.1\%}  & \textbf{0.104} & 0.211 & 74.8\% & 93.2\% \\
\midrule
Uniform baseline   & 97.9\% & 0.260 & ---   & 50.4\% & 99.9\% \\
Marginal baseline  & 4.0\%  & 0.021 & ---   & 96.8\% & 100\%  \\
\midrule
...
The model reproduces the education--income gradient with a mean absolute relative error of 5.1\% (95\% bootstrap CI [3.4\%, 5.9\%]).
Errors are smallest for bachelor's (2.8\%) and PhD (1.3\%) and largest for high school (13.2\%).
...
Bachelor's    & \$78{,}000 & \$80{,}236 & 2.8\%  \\
Master's      & \$98{,}000 & \$95{,}680 & 2.4\%  \\
PhD           & \$120{,}000 & \$118{,}456 & 1.3\% \\
\midrule
\multicolumn{3}{l}{Mean abs.\ relative error} & \textbf{5.1\%} \\
\multicolumn{3}{l}{95\% bootstrap CI} & {[}3.4\%, 5.9\%{]} \\
\bottomrule
\end{tabular}
\end{table}
...
All models recover a strong, statistically precise education--income relationship ($R^2 = 0.87$--0.90), with concave experience profiles (positive linear, negative quadratic term) consistent with the canonical age--earnings lifecycle.
Figure~\ref{fig:ageincome} visualizes this concave age--earnings profile directly: mean income rises through mid-career and then declines, matching the canonical lifecycle.
However, the estimated returns to education (13.3--14.6\% per year) are systematically above the canonical 8--10\% range, reflecting the models' tendency to compress income differences into the education dimension.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\linewidth]{figures/age_income.png}
\caption{Age--income lifecycle profile for employed synthetic records (priors condition), with 95\% confidence bands. The concave profile is consistent with the canonical age--earnings lifecycle.}
\label{fig:ageincome}
\end{figure}
...
Solar Pro 4       & 598 & 0.867 & 0.135 & {[}0.131, 0.140{]} \\
\midrule
Canonical         & --- & ---   & 0.08--0.10 & --- \\
\bottomrule
\end{tabular}
...
Solar Pro 4       & 0.867 & 0.135 & {[}0.131, 0.140{]} \\
\midrule
...
\caption{Post-stratification on the public education distribution reduces the education-distribution TVD to zero by construction while preserving the Mincer $R^2$ (changes within $\pm 0.03$). This validates the paper's central recommendation: LLM joint structure plus marginal calibration.}
...
\midrule
\textbf{Mean change} & --- & --- & --- & $\pm 0.03$ \\
\bottomrule
\end{tabular}
\end{table}

The result is decisive: post-stratification drives the education-distribution TVD to zero by construction, yet the Mincer $R^2$ changes by at most 0.023 (and in two of four models \emph{increases}), confirming that the joint structure is preserved under marginal reweighting.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
  \item \textbf{Priors improve income fidelity.} Across all models, moving from \emph{minimal} to \emph{priors} reduces MARE by 36--77\% (e.g., Solar Pro 4: 21.7\% $\to$ 5.1\%; Ling 3.0 Flash: 16.7\% $\to$ 9.4\%).
  \item \textbf{Priors have limited effect on $R^2$.} Moving from \emph{minimal} to \emph{priors} changes the Mincer $R^2$ by at most 0.04 for each model, indicating that the joint structure is largely \emph{latent}---the models already encode the education--income correlation without explicit instruction.
  \item \textbf{Structural constraints improve consistency but not fidelity.} The \emph{structural} condition consistently improves the consistency metric $P(\text{income}=0 \mid \text{not employed})$ (e.g., Solar Pro 4: 37.3\% $\to$ 100.0\%) but does not reliably reduce MARE---it increases MARE for three of four models and reduces it only marginally for Solar Pro 4.
\end{enumerate}
...
\multirow{3}{*}{\shortstack[l]{Solar\\Pro 4}}
  ...
  & priors     & \textbf{5.1\%}  & \textbf{0.104} & 0.867 & 0.135 \\
\midrule
\multicolumn{2}{l}{Uniform baseline}  & 97.9\% & 0.260 & 0.004 & $-0.010$ \\
\multicolumn{2}{l}{Marginal baseline} & 4.0\%  & 0.021 & 0.229 & 0.117 \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:mare} visualizes the MARE across all conditions and baselines with bootstrap confidence intervals, making the ablation pattern visually explicit: the \emph{priors} condition consistently achieves the lowest MARE for every model, while the marginal baseline (which is handed the correct marginals) achieves the lowest overall.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{figures/mare_comparison.png}
\caption{Income fidelity (MARE) across prompt conditions and baselines, with 95\% bootstrap confidence intervals. The dashed line marks the 10\% threshold.}
\label{fig:mare}
\end{figure}
...
Figure~\ref{fig:consistency} shows this pattern across all prompt conditions, confirming that the employment--hours--income consistency is robust to the prompt while the income-zero-for-unemployed constraint is the most fragile.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{figures/consistency_heatmap.png}
\caption{Internal consistency across prompt conditions and baselines. The employment--hours--income dimensions are near-perfect; the income-zero-for-unemployed constraint is the weakest, especially for DeepSeek.}
...
\label{fig:unemp}
\end{figure}
Table~\ref{tab:unemployment} shows that all models dramatically overstate unemployment for lower education levels, with MAREs ranging from 3.4$\times$ to 7.8$\times$ the reference values.
...
\midrule
\textbf{MARE}     & --- & 5.5$\times$ & 14.3$\times$ & 3.4$\times$ & 7.8$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
The Mincer $R^2$ is remarkably stable: CV $< 3.4\%$ for all (model, condition) pairs, with most below 2\%.
...
The education--income gradient, the employment--hours--income consistency, and the concave age--earnings profile are all captured well, as evidenced by MAREs of 5.1--11.6\% and Mincer $R^2$ values of 0.87--0.90.
Yet the models overstate the employment rate (62--83\% vs.\ 63\%), overstate advanced degrees (PhD 9.5--11.8\% vs.\ 3.3\%), and dramatically distort unemployment rates.
...
The models over-represent advanced degrees (PhD 9.5--11.8\% vs.\ 3.3\%)---consistent with a training corpus that discusses PhD holders far more than their 3.3\% population share would warrant.
...
The \emph{marginal} baseline, which is handed the correct marginals, trivially beats the LLMs on MARE (4.0\% vs.\ 5.1--11.6\%) and TVD (0.02 vs.\ 0.10--0.19)---but it fails to capture any joint structure, fitting a Mincer regression with only $R^2=0.23$.
...
The ablation results (Table~\ref{tab:ablation}) have a practical implication for practitioners: providing explicit economic priors in the prompt substantially improves income fidelity (MARE reduction of 36--77\%) but has negligible effect on the downstream econometric $R^2$ ($< 0.04$ change).
...
The cross-seed analysis (Table~\ref{tab:stability}) provides important reassurance: the Mincer $R^2$ is stable across seeds (CV $< 3.4\%$), indicating that the econometric utility of the synthetic data is not an artifact of a particular random seed.
...
Solar Pro 4 stands out for achieving the lowest MARE (5.1\%) and highest internal consistency (100\% on all four consistency metrics), suggesting that it may be particularly well-suited for applications requiring both fidelity and logical coherence.
...
Sixth, all models exhibit the ``PhD overrepresentation'' bias---generating 9.5--11.8\% PhD holders versus the 3.3\% Census figure---which may reflect training data composition or a systematic tendency to overrepresent highly educated individuals.
...
Values range from 0.149 (DeepSeek) to 0.211 (Solar Pro 4), indicating that the synthetic distributions are shifted by 15--21\% of the reference median.
